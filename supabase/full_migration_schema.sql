-- =====================================================
-- 아이니의원(AINE) 통합 Supabase 스키마 (Full Migration)
-- 생성일: 2026-01-22
-- 버전: 2.0
-- 설명: 전체 데이터베이스 이관을 위한 통합 SQL 스크립트
-- =====================================================
...
-- 2.1 is_staff() 함수: 스태프 여부 확인 (RLS 정책에서 사용)
...
CREATE TABLE IF NOT EXISTS public.consents (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  type TEXT NOT NULL,             -- 'privacy', 'marketing', 'third_party', 'face_style'
  agreed BOOLEAN NOT NULL DEFAULT FALSE,
  agreed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ip_address TEXT,
  user_agent TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT consents_user_type_unique UNIQUE (user_id, type)
);
...
-- =====================================================
-- SECTION 11: FACE STYLE SYSTEM (v2.0)
-- =====================================================

-- 11.1 face_style_sessions 테이블
...
-- 13.6 Clinical Notes & Treatment Plans
...
-- 13.11 Intake Sessions & Messages
...
-- 13.15 Marketing Events & Conversions
...
-- 13.18 Face Style Sessions
CREATE POLICY "Users can view own face style sessions" ON public.face_style_sessions
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own face style sessions" ON public.face_style_sessions
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own face style sessions" ON public.face_style_sessions
  FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own face style sessions" ON public.face_style_sessions
  FOR DELETE USING (auth.uid() = user_id);

-- 13.19 Face Style Variants
CREATE POLICY "Users can view own face style variants" ON public.face_style_variants
  FOR SELECT USING (
...
-- 14.5 chat_sessions 트리거
CREATE TRIGGER update_chat_sessions_updated_at
  BEFORE UPDATE ON public.chat_sessions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 14.6 push_tokens 트리거
...
BEGIN
  RAISE NOTICE '============================================';
  RAISE NOTICE '아이니의원 통합 스키마 설치 완료!';
  RAISE NOTICE '테이블: 24개';
  RAISE NOTICE 'RLS 정책 및 트리거 적용 완료';
  RAISE NOTICE '============================================';
END $$;
